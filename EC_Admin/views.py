...
# Helper function to get admin username and image from session/DB
def get_admin_context(request):
    """Return a dict with username and image for template context"""
    admin_id = request.session.get('admin_id')
    if not admin_id:
        return {'username': 'Admin', 'image': None}
    try:
        a = EC_Admins.objects.get(ecadmin_id=admin_id)
        return {'username': admin_id, 'image': a.ecadmin_image}
    except EC_Admins.DoesNotExist:
        return {'username': admin_id, 'image': None}
...
def adminhome(request):
    ...
    try:
        a = EC_Admins.objects.get(ecadmin_id=admin_id)
        return render(request,'admin/adminhome.html',{'username':admin_id,'image':a.ecadmin_image})
    except EC_Admins.DoesNotExist:
        # Try to create EC_Admin record from superuser data
        try:
            user = User.objects.get(username=admin_id)
            admin = EC_Admins(
                ecadmin_id=admin_id,
                firstname=user.first_name or admin_id,
                ...
                email=user.email or "admin@election.com",
                ecadmin_image="ECAdminImage/default.jpg"
            )
            admin.save()
            messages.info(request, 'Admin profile created. Please update your details in Admin Profile.')
            return render(request,'admin/adminhome.html',{'username':admin_id,'image':admin.ecadmin_image})
        except Exception as e:
            messages.error(request, f'Error: Please add admin details. {str(e)}')
...
def addvoter(request):
    return render(request, 'admin/addvoter.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def add_voter(request):
    if (request.method == 'POST'):
        ...
        if age < 18:
            messages.error(request, f'Voter must be at least 18 years old. Current age: {age} years.')
            return render(request, 'admin/addvoter.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
        
        else:
            add_voter = Voters(voterid_no=voterid_no, name=name, father_name=father_name, gender=gender,
                               dateofbirth=dob, address=address, mobile_no=mobile_no, state=state,
                               pincode=pincode, parliamentary=parliamentary, assembly=assembly, voter_image=voter_image)
            add_voter.save()
            messages.success(request, f'Voter {name} added successfully (Age: {age} years)')
            return render(request, 'admin/addvoter.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def addcandidate(request):
    return render(request, 'admin/add candidate.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
        ...
        if (Candidates.objects.filter(candidate_id=candidate_id).exists()):
            ...
            if parliamentary:
                add_candidate = Candidates(candidate_id=candidate_id, name=name, father_name=father_name, gender=gender,
                                           ...
                                           pincode=pincode, parliamentary=parliamentary,
                ...
                add_candidate = Candidates(candidate_id=candidate_id, name=name, father_name=father_name, gender=gender,
                                           dateofbirth=dateofbirth, address=address, mobile_no=mobile_no, state=state,
                                           pincode=pincode, assembly=assembly,
                                           candidate_party=candidate_party, candidate_image=candidate_image,
                                           party_image=party_image,affidavit=affidavit)
                add_candidate.save()
            else:
                messages.error(request, 'Please enter either Parliamentary or Assembly constituency')
                return render(request, 'admin/add candidate.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def generateelection(request):
    return render(request, 'admin/generateelection.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
    ...
    if request.method == 'POST':
        ...
        if (Election.objects.filter(election_id=election_id).exists()):
            ...
            if election_type == 'PC-GENERAL':
                c = Candidates.objects.filter(state=state, parliamentary__isnull=False)
                for i in c:
                ...
                c = Candidates.objects.filter(state=state, assembly__isnull=False)
                for i in c:
                    candidates_votes = Votes(election_id=election_id, candidate_id=i.candidate_id,
                                             candidate_name=i.name, candidate_party=i.candidate_party, state=i.state)
                    candidates_votes.save()
...
@staff_member_required(login_url='home')
def voting(request):
    return render(request, 'admin/voting.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
    ...
    if request.method == 'POST':
        ...
        if (Voted.objects.filter(election_id=election_id, voter_id=vid).exists()):
            ...
            if v.has_voted == 'no':
                ...
                return render(request, 'admin/voting.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image'], 'where':v.where_voted})
        else:
...
def completeelection(request):
    if Election.objects.filter(status='active'):
        ...
        return render(request, 'admin/completeelection.html', {'elections': e,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
    else:
        messages.info(request, 'No elections running')
        return render(request, 'admin/completeelection.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def modifyelection(request):
    if Election.objects.filter(status='active'):
        e=Election.objects.filter(status='active')
        return render(request, 'admin/modifyelection.html', {'elections': e,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
    else:
        messages.info(request, 'No elections running')
        return render(request, 'admin/modifyelection.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def generateresult(request):
    return render(request, 'admin/generateresult.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def viewcandidatesforaddvote(request):
    if request.method == 'POST':
        ...
        # No longer filtering by constituency as it was removed from Votes model
        candidates = Votes.objects.filter(election_id=viewcandidatesforaddvote.election_id, state=state)
        return render(request, 'admin/addevmvotes.html', {'candidates': candidates,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def submitevmvote(request):
    ...
    return render(request, 'admin/addevmvotes.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def viewresult(request):
    ...
    return render(request, 'admin/viewresult.html', {'elections': elections,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def view_result(request):
    if request.method=="POST":
        ...
        if resulttype=="partywise":
            ...
            for i in won:
                total+=i
            return render(request, 'admin/viewpartywise.html', {'total':total,'parwon':parwon,'electionid': election_id,'state':estate,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
        elif resulttype=="constituencywise":
            # Removed constituency filtering as field was removed from Votes model
            return render(request, 'admin/viewresultconwise.html', {'electionid':election_id,'state':estate,'constituency':[],'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def view_result_filter(request):
    if request.method=="POST":
        ...
        return render(request, 'admin/viewresultconwise.html', {'totalonline':totalonline,'totalevm':totalevm,'totalvotes':totalvotes,'result':finalresult,'electionid':election_id,'state':estate,'constituency':[],'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})


@login_required(login_url='home')
@staff_member_required(login_url='home')
def generatereport(request):
    return render(request, 'admin/generatereport.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def generate_report(request):
    ...
    parliamentary = request.POST.get('ParliamentaryConstituency', '').strip()
    assembly = request.POST.get('AssemblyConstituency', '').strip()
    # For now, using all voters in state; constituency field can be added back to Voted if needed
    if Voted.objects.filter(election_id=election_id, state=state):
        vvotedall=Voted.objects.filter(election_id=election_id, state=state)
        e_male=0
        e_female=0
        e_others=0
        e_total=0
        ...
        for i in vvotedall:
            ...
            if vdetail.gender == 'Male':
                ...
                e_others=e_others+1
        has_voted='yes'
        if Voted.objects.filter(election_id=election_id, state=state, has_voted=has_voted):
            vvotedyes=Voted.objects.filter(election_id=election_id, state=state, has_voted=has_voted)
            v_male=0 
            v_female=0
            v_others=0
            v_total=0
            ...
            if e_total!=0:
                ...
                p_total=0
                poll_total=float("{:.2f}".format(p_total))
            # Create report with state; constituency field can be added back to Reports if needed
            report=Reports(election_id=election_id, state=state, constituency='', electors_male=e_male, electors_female=e_female, electors_others=e_others, electors_total=e_total, voters_male=v_male, voters_female=v_female, voters_others=v_others, voters_total=v_total, poll_male=poll_male, poll_female=poll_female, poll_others=poll_others, poll_total=poll_total)
            report.save()
...
def viewreport(request):
    elections = Election.objects.all()
    return render(request, 'admin/viewreport.html', {'elections': elections,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})


@login_required(login_url='home')
def view_report(request):
    election_id = request.POST['e_id']
    # No longer filtering by constituency; fetch all reports for election
    report = Reports.objects.filter(election_id=election_id)
    elections = Election.objects.all()
    return render(request, 'admin/viewreport.html', {'report': report, 'elections':elections,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})


@login_required(login_url='home')
@staff_member_required(login_url='home')
def viewcandidate(request):
    return render(request, 'admin/view candidate.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def view_candidate(request):
    if request.method == 'POST':
        ...
        if parliamentary:
            ...
            if candidates:
                return render(request, 'admin/view candidate.html', {'constituency':parliamentary,'candidates': candidates,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
            else:
            ...
            if candidates:
                return render(request, 'admin/view candidate.html', {'constituency':assembly,'candidates': candidates,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
            else:
                messages.info(request, 'No Candidate Found')
                return render(request, 'admin/view candidate.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
        else:
            messages.info(request, 'Please enter a Parliamentary or Assembly constituency')
            return render(request, 'admin/view candidate.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def viewvoter(request):
    return render(request, 'admin/view voter.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def view_voter(request):
    if request.method == 'POST':
        ...
        assembly = request.POST.get('AssemblyConstituency', '').strip()
        if parliamentary:
            voters = Voters.objects.filter(state=state, parliamentary=parliamentary)
            return render(request, 'admin/view voter.html', {'voters': voters,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
        elif assembly:
            voters = Voters.objects.filter(state=state, assembly=assembly)
            return render(request, 'admin/view voter.html', {'voters': voters,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
        else:
            messages.info(request, 'Please enter a Parliamentary or Assembly constituency')
            return render(request, 'admin/view voter.html', {'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def editvoter(request):
    return render(request, 'admin/edit voter.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
        ...
        if request.POST.get('editvoter'):
            ...
            if Voters.objects.filter(voterid_no=voter_id):
                ...
                return render(request, 'admin/edit voterdetails.html', {'voterid_no': voter.voterid_no, 'name': voter.name,
                                                                ...
                                                                'assembly': voter.assembly, 'voter_image': voter.voter_image,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
            else:
                messages.info(request,'Voter not found')
                return render(request,'admin/edit voter.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def editcandidate(request):
    return render(request, 'admin/edit candidate.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
    ...
    if request.method == 'POST':
        if request.POST.get('editcandidate'):
            ...
            if Candidates.objects.filter(candidate_id=candidate_id):
                ...
                # Removed constituency field logic; show parliamentary and assembly if present
                return render(request, 'admin/edit candidatedetails.html',
                            ...
                            'parliamentary': candidate.parliamentary, 'assembly': candidate.assembly,
                            'candidate_image': candidate.candidate_image, 'candidate_party': candidate.candidate_party,
                            'party_image': candidate.party_image,'affidavit':candidate.affidavit,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
            else:
...
def viewcomplain(request):
    if Complain.objects.filter(viewed=False, replied=False):
        ...
        return render(request, 'admin/viewcomplain.html', {'complain': allcomplain,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
    else:
        messages.info(request,'No New Complains')
        return render(request, 'admin/viewcomplain.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def replycomplain(request):
    if Complain.objects.filter(viewed=False, replied=False):
        allcomplain=Complain.objects.filter(viewed=False, replied=False)
        return render(request, 'admin/replycomplain.html', {'complain': allcomplain,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
    else:
        messages.info(request,'No New Complains')
        return render(request, 'admin/viewcomplain.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
...
def reply_complain(request):
    if request.method == "POST":
        ...
        complain.save()
        allcomplain = Complain.objects.filter(viewed=False, replied=False)
        return render(request, 'admin/replycomplain.html',{'complainid':complain_id,'complain':allcomplain,'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})


@login_required(login_url='home')
@staff_member_required(login_url='home')
def achangepassword(request):
    return render(request, 'admin/achangepassword.html',{'username':get_admin_context(request)['username'],'image':get_admin_context(request)['image']})
